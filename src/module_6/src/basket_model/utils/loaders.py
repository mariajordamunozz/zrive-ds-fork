...
def load_orders() -> pd.DataFrame:
    ...
    if "ordered_items" in orders.columns and "item_ids" not in orders.columns:
        orders = orders.rename(columns={"ordered_items": "item_ids"})
...
def get_mean_item_price() -> float:
    ...
    price_column = "our_price" if "our_price" in inventory.columns else "price"
    return inventory[price_column].mean()
